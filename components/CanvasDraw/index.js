...
import { getRoom, socket } from "@/socket"; // Yoyo
...
export default class CanvasDraw extends PureComponent {
  ...
  socketOn = () => {
    ...
    socket.on("save line", (points, brushColor, brushRadius) => {
      const triggerEmit = false;
      this.saveLine({ points, brushColor, brushRadius }, triggerEmit);
    });
    socket.on("undo", () => {
      this.undo(false);
    });
  ...
  drawPoints = ({ points, brushColor, brushRadius }, triggerEmit = true) => {
    // Yoyo
  ...
  saveLine = ({ points, brushColor, brushRadius } = {}, triggerEmit = true) => {
    if (triggerEmit) {
      socket.emit(
        "draw points",
        [...this.points],
